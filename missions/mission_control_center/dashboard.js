...
class Dashboard {
  ...
  getStatus() {
    return {
      mission: 'mission_control_center',
      role: 'dashboard',
...
if (require.main === module) {
  ...
  if (command === 'run') {
    const result = instance.generateDashboard();
    console.log('✅ Dashboard ran successfully');
    console.log('Status:', JSON.stringify(instance.getStatus(), null, 2));
  } else if (command === 'status') {
    console.log(JSON.stringify(instance.getStatus(), null, 2));
  } else {
    console.log('Usage: node dashboard.js [run|status]');
  }
}
