// ============================================================
// UNIFIED FINANCE & PORTFOLIO MANAGER v2.0
// Merged: portfolio_manager + research_agent + daily_market_report + research_pipeline
// ============================================================
...
class UnifiedFinanceManager {
  ...
  async runFullCycle() {
    ...
    try {
      ...
      // 2. Alternative Data (Fear & Greed, etc.)
      ...
      // 10. Send Email Report (if configured)
  ...
  async refreshMarketData() {
    ...
    try {
      // Use market_data_service.js (Twelve Data + fallbacks)
  ...
  async fetchAlternativeData() {
    ...
    try {
      const output = execSync('node fetch_alternative_data.js --json', {
        cwd: 'C:\\Users\\quent\\.openclaw\\workspace\\investment_fund\\scripts',
        encoding: 'utf8',
        timeout: 60000
      });
  ...
  async runTechnicalAnalysis() {
    ...
    for (const asset of ALL_ASSETS.slice(0, 4)) { // Limit to save API calls
      try {
        const output = execSync(`node enhanced_ta_analysis.js ${asset} --json`, {
          cwd: 'C:\\Users\\quent\\.openclaw\\workspace\\mission_control',
          encoding: 'utf8',
          timeout: 30000
        });
  ...
  async updatePortfolioTracking() {
    ...
    try {
      ...
      // Read portfolio positions (if file exists)
  ...
  async runResearchPipeline() {
    ...
    for (const topic of RESEARCH_TOPICS.slice(0, 3)) { // Limit to save time
      try {
  ...
  async searchTopic(topic) {
    // Simplified search - could be expanded with arXiv, PubMed, etc.
    return {
  ...
  async generateDailyReport() {
    ...
    const report = [
      ...
      ...(this.results.riskAlerts?.length > 0 
        ? this.results.riskAlerts.map(a => `- ⚠️ ${a.message}`)
        : ['- No alerts']),
      ``,
      `## Research`,
  ...
  async updateDashboard() {
    console.log('\n🖥️ [8/10] Updating Dashboard...');
    
    try {
      // Update index.html with latest prices
      // (Implementation would edit the HTML file)
  ...
  async sendEmailReport() {
    ...
    // Requires Gmail API setup (googleapis package)
    // This is optional - skip if not configured
    console.log('   ⚠️ Email sending requires Gmail API (optional)');
  }
