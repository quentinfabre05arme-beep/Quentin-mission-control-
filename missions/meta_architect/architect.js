...
if (require.main === module) {
  const instance = new Architect();
  const command = process.argv[2];
  if (command === 'run') {
    const result = instance.analyzeStructure();
    console.log('✅ Architect ran successfully');
    console.log('Status:', JSON.stringify(instance.getStatus(), null, 2));
  } else if (command === 'status') {
    console.log(JSON.stringify(instance.getStatus(), null, 2));
  } else {
    console.log('Usage: node architect.js [run|status]');
  }
}
